...
export class CircuitCallButton extends HTMLElement {
  ...
  async _init() {
    ...
    this._client.addEventListener('callStatus', e => {
      ...
      this.dispatchEvent(new CustomEvent('callStatus', { detail: e.call }));
    });
    ...
    this._client.addEventListener('callEnded', e => {
      ...
      this.dispatchEvent(new CustomEvent('callEnded', { detail: e.call }));
  ...
  async _makeCall() {
    ...
    try {
      await this._connect();
      this.call = await this._client.makeCall(this._target, {audio: true}, true);
    } catch (err) {
      this.removeAttribute('disabled');
